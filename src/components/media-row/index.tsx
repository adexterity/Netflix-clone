  ...
  return <div className="h-40 space-y-0.5 md:space-y-2 px-4">
    ...
    <div className="group relative md:-ml-2">
        <div className="flex items-center scrollbar-hide space-x-0 5 overflow-x-scroll md:space-x-2.5 md:p-2">
            {
                medias && medias.length && medias.filter(item=> item.backdropdrop_path !== null && item.poster_path !== null).map(mediaItem=> <MediaItem title = {title} key={mediaItem.id} media = {mediaItem} />)
            }
        </div>
    </div>
  </div>;
